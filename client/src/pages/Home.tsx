      ...
      <div style={{ textAlign: 'center', color: '#f5e7d9', paddingTop: 80 }}>
        ...
        <div style={{ margin: '30px auto', maxWidth: 320 }}>
          <input
            ...
            placeholder="用户名"
            style={{ display: 'block', width: '100%', marginBottom: 12, padding: '10px 14px', borderRadius: 8, border: '1px solid #555', background: '#1a2a1a', color: '#f5e7d9', fontSize: '1rem' }}
          />
          <input
            type="password"
            value={password}
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            <button onClick={handleLogin} style={btnStyle('#e67e22')}>登录</button>
            <button onClick={handleRegister} style={btnStyle('#555')}>注册</button>
          </div>
          <div
            onClick={handleGuest}
            style={{ marginTop: 16, color: '#99ccff', cursor: 'pointer', textDecoration: 'underline', fontSize: '0.85rem' }}
          >
            游客模式（不计资产）
          </div>
        </div>
...
function Card({ icon, label, desc, onClick }: { icon: string; label: string; desc: string; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      style={{
        background: 'rgba(0,0,0,0.3)', borderRadius: 16, padding: '24px 16px',
        cursor: 'pointer', border: '2px solid transparent', transition: 'all 0.15s',
      }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLDivElement).style.borderColor = '#ffd966';
